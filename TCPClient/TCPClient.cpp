...
bool TCPClient::loadConfig(){
	...
	string address  = boost::lexical_cast<string>(config_->server.Address.value_);
	unsigned short port = boost::lexical_cast<unsigned short>(config_->server.Port.value_);
...
bool TCPClient::Connect(boost::asio::io_service& IOService){
	if (isConnected == false){
		const std::string s_address = boost::lexical_cast<string>(serverAddress);
		const std::string s_port = boost::lexical_cast<string>(serverPort);		
...
void TCPClient::SendMsgAsync(){
	...
	conn_->asyncSendMsg(messenger_->msgQ_.QOutgoing , boost::bind(&MESSENGER::Messenger::SendDummyMsg, messenger_, boost::asio::placeholders::error, conn_));
}
...
void TCPClient::RcvMsgAsync(){
	...
	conn_->asyncRcvMsg(messenger_->msgQ_.QIncoming , boost::bind(&MESSENGER::Messenger::handleRead, messenger_, boost::asio::placeholders::error, conn_));
}
